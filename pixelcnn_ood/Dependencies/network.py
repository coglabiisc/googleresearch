...
class PixelCNN(distribution.Distribution):
  ...
  def _log_prob(self, value, conditional_input=None, training=None, join_events=True, return_dist=False):
    ...
    if return_dist:
        if join_events:
            return tf.reshape(dist.log_prob(value), image_batch_and_conditional_shape), dist
        else:
            return dist.log_prob(value), dist
    else:
        if join_events:
  ...
  def compute_corrections(self, dataset=None, n_batches=500):
    ...
    if type(dataset) is np.ndarray:
      train_batch = dataset.copy()
      pixel_ll = self.log_prob(train_batch, training=False, join_events=False).numpy().squeeze()
      inp = train_batch.squeeze()
      if self.event_shape[-1] == 3:
        inp = inp.mean(axis=3).round()  # Change here for color
    ...
    for missing_pix in (set(range(pix_range)) -  # Change here for color
                      set(self.corr_dict)):
      self.corr_dict[missing_pix] = f(missing_pix)
    corr_vec = np.vectorize(lambda x: self.corr_dict[x])
    self.correct = lambda x: corr_vec(x.mean(axis=3, keepdims=True).round().astype(np.int32))
  ...
  for i in range (n):
    for j in range (n):
      shuffled[:, i*(patch_shap[1]):(i+1)*(patch_shap[1]), j*(patch_shap[2]):(j+1)*(patch_shap[2])
                         , :] = patches[:,:,:,:,np.squeeze(orders[i*n+j])]
  return shuffled
...
class _PixelCNNNetwork(tf.keras.layers.Layer):
  ...
  def __init__(
    ...
    self._rem_block = rem_block
    self._rem_seq_sc = rem_seq_sc
    self._rem_sc = rem_sc
    ...
    for i in range(self._num_hierarchies):
      for _ in range(self._num_resnet):
        ...
        for stack in ['vertical', 'horizontal']:
          ...
          if self._patch_shuffle is not None:
#             if i > 0:
              temp_shape = tf.shape(layer_stacks[stack][-1])
              layer_stacks[stack][-1] = tf.reshape(tf.numpy_function(patch_shuffle, [layer_stacks[stack][-1]], tf.float32), temp_shape)
    ...
    for i in range(self._num_hierarchies):
      ...
      for j in range(num_resnet):
        ...
        for stack in ['vertical', 'horizontal']:
          ...
          h = Dense(self._num_filters)(h)
          if self._rem_seq_sc is not None:
            if i <= self._rem_seq_sc:
              h = tf.zeros_like(h)
          if self._rem_sc is not None:
            if i in self._rem_sc:
              h = tf.zeros_like(h)
          x = tf.keras.layers.add([h, x], dtype=dtype)
          ...
          if self._patch_shuffle is not None:
#             if i < self._num_hierarchies - 1:
            if not (i == (self._num_hierarchies - 1) and j == (self._num_resnet - 1)):
              temp_shape = tf.shape(upward_pass[stack])
              upward_pass[stack] = tf.reshape(tf.numpy_function(patch_shuffle, [upward_pass[stack]], tf.float32), temp_shape)
      ...
      if i < self._num_hierarchies - 1:
        for stack in ['vertical', 'horizontal']:
          ...
          x = Conv2DTranspose(kernel_size=(kernel_height, kernel_width),
                              kernel_constraint=kernel_constraint)(x)
#           if self._patch_shuffle is not None:
#             temp_shape = tf.shape(x)
#             x = tf.reshape(tf.numpy_function(patch_shuffle, [x], tf.float32), temp_shape)
          upward_pass[stack] = x
        
          if i == self._rem_block:
            upward_pass[stack] = tf.zeros_like(upward_pass[stack])
